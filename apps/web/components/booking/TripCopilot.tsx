...
export default function TripCopilot({
  ...
  if (state.kind === 'collapsed') {
    return (
      <button
        onClick={expand}
        className="w-full flex items-center justify-between px-4 py-3 bg-white border border-[#E5E7EB] rounded-card hover:border-primary/40 hover:bg-[#F0FDF4] transition-colors text-left"
      >
        <div className="flex items-center gap-2">
          <span className="text-xl">✦</span>
          <div>
            <p className="text-sm font-semibold text-[#1A1A1A]">Plan Your Trip</p>
            <p className="text-xs text-[#6B7280]">AI route guide, fuel estimate & top stops</p>
          </div>
        </div>
  ...
  if (state.kind === 'loading') {
    return (
      <div className="w-full flex items-center gap-3 px-4 py-4 bg-white border border-[#E5E7EB] rounded-card">
  ...
  if (state.kind === 'error') {
    return (
      <div className="w-full px-4 py-3 bg-white border border-[#E5E7EB] rounded-card">
  ...
  return (
    <div className="bg-white border border-[#E5E7EB] rounded-card overflow-hidden">
      ...
      >
        <div className="flex items-center gap-2">
          <span className="text-xl">✦</span>
          <p className="text-sm font-semibold text-[#1A1A1A]">Your Trip Plan</p>
        </div>
      ...
      <div className="px-4 pb-4 space-y-4 border-t border-[#E5E7EB]">
        {/* Route summary */}
        <div className="pt-3">
          <p className="text-xs font-semibold text-[#6B7280] uppercase tracking-wider mb-1">Route</p>
          <p className="text-sm text-[#1A1A1A]">{data.route_summary}</p>
        </div>
        ...
        {data.estimated_fuel_cost_eur != null && (
          <div className="flex items-center gap-2">
            ...
            <div>
              <p className="text-xs font-semibold text-[#6B7280] uppercase tracking-wider">Est. Fuel Cost</p>
        ...
        {data.fuel_note && (
          <div className="flex items-start gap-2 p-3 bg-blue-50 border border-blue-100 rounded-md">
            ...
            <p className="text-xs text-blue-700">{data.fuel_note}</p>
          </div>
        )}
        ...
        {data.top_stops.length > 0 && (
          <div>
            <p className="text-xs font-semibold text-[#6B7280] uppercase tracking-wider mb-2">Recommended Stops</p>
            <div className="space-y-2">
              {data.top_stops.map((stop, i) => (
                <div key={i} className="flex gap-3">
                  ...
                  <div>
                    <p className="text-sm font-medium text-[#1A1A1A]">{stop.name}</p>
                    <p className="text-xs text-[#6B7280]">{stop.description}</p>
                  </div>
                </div>
              ))}
            </div>
        ...
        {/* Parking */}
        <div>
          <p className="text-xs font-semibold text-[#6B7280] uppercase tracking-wider mb-1">Parking Tips</p>
          <p className="text-sm text-[#6B7280]">{data.parking_tips}</p>
        </div>
      </div>
    </div>
  )
